...
function translate(io, rp::ResolvedProtoFile, file_map::Dict{String,ResolvedProtoFile}, options)
    ...
    println(io, "# Autogenerated using ProtoBuf.jl v$(PACKAGE_VERSION)")
    println(io, "# original file: ", rp.import_path," (proto", p.preamble.isproto3 ? '3' : '2', " syntax)")
    println(io)
    ...
    if !is_namespaced(p)
        ...
        for path in import_paths(p)
            ...
            if !is_namespaced(dependency)
                ...
                println(io, "include(", _include_stmt_format(proto_script_name(dependency)), ")")
                ...
                println(io, "include(", _include_stmt_format(namespaced_top_include(dependency)), ")")
